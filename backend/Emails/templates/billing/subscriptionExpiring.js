/**
 * Generate subscription expiring email HTML.
 *
 * @param {Object} data
 ...
 * @returns {string}
 */
const generateSubscriptionExpiringHTML = (data) => {
    const {
        userName = "there",
        planName,
        expiresAt,
...
<title>Subscription Expiring - CuratoCV</title>
<style>
...
.alert-box { background-color: #fffbeb; border-left: 4px solid #f59e0b; padding: 16px; margin: 24px 0; border-radius: 4px; }
.alert-box p { margin: 0; color: #92400e; font-size: 14px; }
.cta-button { display: inline-block; background: linear-gradient(135deg, #f59e0b 0%, #d97706 100%); color: white; padding: 14px 32px; border-radius: 6px; text-decoration: none; font-weight: 600; font-size: 16px; margin: 16px 0; text-align: center; }
...
<div class="alert-box"><p>Your <strong>${planName}</strong> subscription will expire on <strong>${expiresFormatted}</strong>. Renew now to keep your premium features active.</p></div>
<p style="text-align:center; margin:24px 0;">
<a href="${billingUrl}" class="cta-button">Renew Subscription</a>
</p>
</div>
<div class="footer">
...
export default generateSubscriptionExpiringHTML;
